 ...
 *
 * Implements the Reference interface to allow using instances for referring to
 * the declaration too.
 */
public class Function extends Reference {
     ...
     * and the declaration of the native C function containing the functions'
     * statements.
     * Since there are no namespaces in C, a globally unique name is required for
     * the
     * declaration of the function itself. For functions, funcName is used for this
     * purpose;
     * for methods, the class name is automatically added as a prefix to funcName
     * once the
    ...
    /**
     * Creates a unique method name used in emitted C code (auxiliary method, called
     * by MPyClass)
     *
     * A method is a function that can only be called in connection with its class.
     * However,
     * since a method is emitted just as a normal function in the C code, the method
     * name must
     * be extended with a prefix at C level and thus be made unique. The constructor
     * of MPyClass
     ...
     *
     * <p>
     * Warning: Do not use this helper method for functions, as functions are not
     * allowed to
    ...
    public String buildCFunction() {
        ...
        body.append("__MPyGetArgsState argHelper = __mpy_args_init(\"" + name + "\", args, kwargs, "
                + positionalArgs.size() + ");\n");
        ...
        if (!receivesPackedPositionalArgs && !receivesPackedKeywordArgs) {
            ...
            // FIXME leave out check for unused arguments only partially (not implemented on
            // the c side too)
