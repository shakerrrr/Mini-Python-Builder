...
public class Assignment implements Statement {
    ...
    public String buildStatement() {
        ...
        if (this.rhs instanceof Reference) {
            // first increment reference count of object we assign since it's now used once
            // more
            // then decrement reference count of object in variable that is reassigned,
            // since it's now used once less.
            // Afterwards perform assignment.
            //
            // Note: Order is important, since for example for the case of `a=a` the
            // variable could reach a count of 0 even
            // if it's still in use afterwards.
            return "__mpy_obj_ref_inc(" + rhs + ");\n" +
                    "__mpy_obj_ref_dec(" + lhs + ");\n" +
                    "__mpy_type_check(" + lhs + "," + rhs + ");\n" +
                    lhs + " = " + rhs + ";\n";
        } else {
            // Note: Order is not important here, since returned values (literals or
            // function calls)
            // have the temporary mechanism and therefore a refCount of +1 until the _inc
